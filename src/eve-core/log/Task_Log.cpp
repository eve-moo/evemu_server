...
const WORD Task_Log::COLOR_TABLE[ COLOR_COUNT ] =
{
...
#else /* !HAVE_WINDOWS_H */
const char* const Task_Log::COLOR_TABLE[ COLOR_COUNT ] =
{
...
void Task_Log::InitializeLogging(std::string logPath, std::string logSubdirectory, std::string logFilename)
{
    // use default logpath if logPath is empty
    if( logPath.empty() )
        logPath = EVEMU_ROOT "/log/";
...
void Task_Log::Log( const char* source, const char* fmt, ... )
{
    va_list ap;
    va_start( ap, fmt );
...
void Task_Log::Error( const char* source, const char* fmt, ... )
{
    va_list ap;
    va_start( ap, fmt );
...
void Task_Log::Warning( const char* source, const char* fmt, ... )
{
    va_list ap;
    va_start( ap, fmt );
...
void Task_Log::Success( const char* source, const char* fmt, ... )
{
    va_list ap;
    va_start( ap, fmt );
...
void Task_Log::Debug( const char* source, const char* fmt, ... )
{
//#ifndef NDEBUG
...
bool Task_Log::SetLogfile( const char* filename )
{
    MutexLock l( mMutex );
...
bool Task_Log::SetLogfile( FILE* file )
{
    MutexLock l( mMutex );
...
void Task_Log::PrintMsg( Color color, char pfx, const char* source, const char* fmt, va_list ap )
{
    if( !m_initialized )
        return;
...
void Task_Log::Print( const char* fmt, ... )
{
    va_list ap;
    va_start( ap, fmt );
...
void Task_Log::PrintVa( const char* fmt, va_list ap )
{
    MutexLock l( mMutex );
...
void Task_Log::SetColor( Color color )
{
...
void Task_Log::SetLogfileDefault(std::string logPath)
{
    MutexLock l( mMutex );
