...
bool SystemDB::LoadSystemDynamicEntities(uint32 systemID, std::vector<DBSystemDynamicEntity> &into) {
    ...
    if(!DBcore::RunQuery(res,
        ...
        "   0,"//"   srvCorporation.allianceID,"
        "   x,"
        "   y,"
        "   z"
        " FROM srvEntity, invTypes, invGroups"//, srvCharacter, srvCorporation"
        " WHERE"
        "        srvEntity.typeID=invTypes.typeID"
        "    AND invTypes.groupID=invGroups.groupID"
        "    AND invGroups.categoryID NOT IN (%d,%d)"
        //"   AND srvCharacter.characterID = srvEntity.ownerID"
        //"   AND srvCorporation.corporationID = srvCharacter.corporationID"
        "    AND locationID=%u",
        //excluded categories:
            //celestials:
